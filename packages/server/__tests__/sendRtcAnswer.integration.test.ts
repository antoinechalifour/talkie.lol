...
describe("sendRtcAnswer", () => {
  ...
  it("should respond with a success", async () => {
    ...
    // When
    const response = await senderClient.sendRtcAnswer(recipientId, answer);
    await flushPromises();

    // Then
    expect(response.data!.sendRtcAnswer).toMatchObject({
      success: true,
    });
  });
  ...
  it("should notify the recipient", async () => {
    ...
    // When
    await senderClient.sendRtcAnswer(recipientId, answer);
    await flushPromises();
